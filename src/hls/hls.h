...
static bool check_extend_args(const std::map<std::string, std::string> * j) {
    if (!j) {
        return false;
    }
...
private:
    std::string outdir;
    const std::map<std::string, std::string> *extends_args;
...
public:
    HlsMuxer(const char*  url, const char*  outdir, const std::map<std::string, std::string> * ext_args);
